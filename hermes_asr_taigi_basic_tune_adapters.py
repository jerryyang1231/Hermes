...
"""
CUDA_VISIBLE_DEVICES=2 python -u hermes_asr_taigi_basic_tune_adapters.py config/audio-text/hermes_asr_taigi_basic_tune_adapters.yaml
"""
...
class DistillWhisperModule(LightningModule):
    def __init__(self, cfg, model_name, lang) -> None:
        ...
        # (B) 設定 Student: 凍結所有，只解凍 adapter 相關層
        print("Setting up Student: Unfreezing only adapter layers.")
        for p in self.student.parameters():
            p.requires_grad = False
            
        student_trainable_keywords = ["adapter"]
        for n, p in self.student.named_parameters():
            if any(keyword in n for keyword in student_trainable_keywords):
                p.requires_grad = True
                print(f"Unfreezing Student param: {n}")
    ...
    def configure_optimizers(self):
        ...
        # 呼叫我們新寫的 adapter optimizer，只訓練 student 的 adapter
        print("Setting up Student's optimizer using whisper_adapter_optimizer...")
        student_cfg = types.SimpleNamespace(**self.cfg.__dict__)
        student_cfg.learning_rate = getattr(self.cfg, 'learning_rate_student', 1e-4)

        optimizer_student, scheduler_student = whisper_adapter_optimizer(

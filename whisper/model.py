...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        if self.add_gated_x_attn != 0: 
            ...
            if collect_feats:
                collected_features['gated_x_attn'] = gated_feats

        if self.add_adapter:
            # 接收來自 Adapter 的兩個回傳值：與殘差相加後的 x，以及純粹的 adapter 輸出
            x, pure_adapter_out = self.adapter(x)
            if collect_feats:
                # 儲存純粹的 adapter 輸出
                collected_features['adapter'] = pure_adapter_out
...
class TextDecoder(nn.Module):
    def __init__(
        ...
        add_gated_x_attn: int, bert_hidden_size: int, num_langs: int, add_adapter: bool, adapter_dim: int,
    ):
        super().__init__()

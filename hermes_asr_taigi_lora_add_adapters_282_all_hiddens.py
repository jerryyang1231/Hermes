...
"""
CUDA_VISIBLE_DEVICES=2 python -u hermes_asr_taigi_lora_add_adapters_282_all_hiddens.py config/audio-text/hermes_asr_taigi_lora_add_adapters_282_all_hiddens.yaml
"""
...
class DistillWhisperModule(LightningModule):
    def __init__(self, cfg, model_name, lang) -> None:
        ...
        # --- Student 設定流程 ---
        print("Loading student model with adapters...")
        # 1. 建立一個 *同時包含* Adapter 的 Student 空殼
        ...
        # --- 手動解凍 Adapter 的參數 ---
        # 因為 get_peft_model 預設會凍結所有非 LoRA 的參數，包括我們剛加入的 adapter
        print("Manually unfreezing adapter parameters...")
        for name, param in self.student.named_parameters():
            if "adapter" in name:
                param.requires_grad = True
                
        print("--- Trainable params after LoRA + Adapter unfreeze ---")
        self.student.print_trainable_parameters()
